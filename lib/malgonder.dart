...
class SaleLineGonder { // Adını dəyişdirərək potensial konfliktin qarşısını alırıq
  final int productId;
  final int telebnameLineIdn;
  ...
  SaleLineGonder({
    ...
    required this.telebnameLineIdn,
  ...
  SaleLineGonder copyWith({
    int? productId,
    int? telebnameLineIdn,
    String? barcode,
    String? name,
    double? price,
    double? qty,
  ...
  {
    return SaleLineGonder(
      productId: productId ?? this.productId,
      telebnameLineIdn: telebnameLineIdn ?? this.telebnameLineIdn,
  ...
  factory SaleLineGonder.fromMap(Map<String, dynamic> m) {
    return SaleLineGonder(
      productId: (m['productId'] as num).toInt(),
      telebnameLineIdn: (m['telebnameLineIdn'] as num? ?? 0).toInt(), // Null yoxlaması
      barcode: (m['barcode'] ?? '').toString(),
      name: (m['name'] ?? '').toString(),
      price: (m['price'] as num).toDouble(),
      qty: (m['qty'] as num).toDouble(),
      unit: (m['unit'] ?? m['unit_name'] ?? '').toString(), // Hər iki varianta da baxır
      qeyd: (m['qeyd'] ?? '').toString(),
      addCount: (m['addCount'] as num? ?? 1).toInt(),
...
class MalgonderPage extends StatefulWidget {
  final TelebHeader? satisHeader;

  const MalgonderPage({super.key, this.satisHeader});

  @override
  State<MalgonderPage> createState() => _MalgonderPageState();
}

class _MalgonderPageState extends State<MalgonderPage> {
  ...
  // Satış sətirləri
  final List<SaleLineGonder> _lines = [];
...
// (istəsəniz duplikat skanların qarşısını almaq üçün)
  ...
  static const String _draftKey = 'malgonder_draft';
  ...
  void initState() {
    ...
    _loadDraft().then((_) {
      if (widget.satisHeader != null) {
        _applySatisHeader(widget.satisHeader!);
      }
      _loadLookups();
  ...
  // === ADDIM 2: Sənəd məlumatlarını səhifəyə tətbiq edən funksiya ===
  void _applySatisHeader(TelebHeader header) {
    setState(() {
      _telebnameNomresiCtrl.text = header.documentNo ?? '';
      _sorguNomresiCtrl.text = header.contract?.toString() ?? '';
      _kontraAdiCtrl.text = header.customerName ?? '';
      _layiheAdiCtrl.text = header.layiheadi ?? '';
      _isinAdiCtrl.text = header.isintesviri ?? '';
      _emekdasAdiCtrl.text = header.ekspeditor ?? '';
      _noteCtrl.text = header.note ?? '';

      // Tələbnamədəki çıxış anbarını burdakı çıxış anbarına mənimsət
      _selectedWarehouseId = header.depo;

      // Sətirləri təmizlə və yenilərini əlavə et
      _lines.clear();
      for (var line in header.lines) {
        // Yalnız balansı (göndərilməmiş miqdarı) olan sətirləri əlavə et
        final balance = (line.quantity ?? 0) - (line.quantity_given ?? 0);
        if (balance > 0) {
          _lines.add(SaleLineGonder(
            productId: line.idn ?? 0, // Sətirdəki məhsulun idn-i
            telebnameLineIdn: line.idn ?? 0, // Tələbnamə sətirinin öz idn-i
            barcode: line.barcode ?? '',
            name: line.malName ?? 'Adsız məhsul',
            price: 0, // Göndərmədə qiymət 0 olur adətən
            qty: balance, // Göndəriləcək miqdar qalıq qədərdir
            unit: line.vahid ?? '',
            qeyd: '',
          ));
        }
    ...
    try {
      ...
      setState(() {
        ...
        final arr = (m['lines'] as List<dynamic>? ?? [])
            .map((e) => SaleLineGonder.fromMap(e as Map<String, dynamic>))
            .toList();
        _lines
          ..clear()
          ..addAll(arr);
...
*/
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      if (resp.statusCode >= 200 && resp.statusCode < 300) {
        ...
        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Malın göndərilməsi sənədi təsdiqləndi')),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Malın göndərilməsi'),
