...
class SatisPage extends StatefulWidget {
  const SatisPage({super.key});

  @override
  State<SatisPage> createState() => _SatisPageState();
}

class _SatisPageState extends State<SatisPage> {
...
// (istəsəniz duplikat skanların qarşısını almaq üçün)
  ...
  static const String _draftKey = 'satis_draft';
  ...
  Future<void> _loadLookups() async {
    ...
    try {
      ...
      // Əgər hər hansı bir cavabda status != 200-dürsə, xəta at
      ...
      final userJson = jsonDecode(responses[3].body) as List;
      final classJson = jsonDecode(responses[4].body) as List;
      final propJson = jsonDecode(responses[5].body) as List;
  ...
  Future<void> _downloadMallar() async {
    ...
    final url = Uri.parse('http://$globalIp:$globalPort/mallarqaliqfifo?userId=$_selectedWarehouseId');
    try {
                  ...
                  Expanded(
                    child: filtered!.isEmpty
                      ...
                      itemBuilder: (context, index) {
                        ...
                        final qaliq = (m?['fifo_qaliq'] ?? '').toString();
          ...
          builder: (context, setState) {
            return AlertDialog(
                      ...
                      children: [
                        ...
                        Expanded(
                          child: TextField(
                            ...
                            keyboardType: const TextInputType.numberWithOptions(decimal: true),
                            onTap: () => discountAmountCtrl.selection =
                                TextSelection(baseOffset: 0, extentOffset: discountAmountCtrl.text.length),
                            onChanged: (_) {
                              setState(() {
                                calculateTotal();
                              });
              ...
              actions: [
                ...
                FilledButton(
                  onPressed: () {
                    final enteredPrice = double.tryParse(priceCtrl.text.replaceAll(',', '.')) ?? price;
  ...
  Future<bool> _askPrintConfirmation() async {
    ...
    final res = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        title: const Text('Çap'),
        content: const Text('Qəbzi çap etmək istəyirsiniz?'),
        actions: [
          TextButton(
    ...
    return res ?? false; // burada istifadəçinin seçimindən asılıdır
  }
  ...
  Future<void> _submit() async {
    ...
    if (_selectedCustomerId == null ||  _selectedWarehouseId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Müştəri/Anbar seçilməyib.')),
      );
      return;
    }
    ...
    final payload = {
      ...
      'customerId': _selectedCustomerId,
      'edvId': _selectedCashDeskId,
      'warehouseId': _selectedWarehouseId,
      'percent': double.tryParse(_discountPercentController.text) ?? 0,
      'discount': double.tryParse(_discountAmountController.text) ?? 0,
      ...
      'lines': _lines.map((e) => {
        ...
        'qty': e.qty,
        'price': e.price,
        'mpercent': e.discountPercent,
        'mdiscount': e.discountAmount,
        'mebleg': (e.price - (e.price * e.discountPercent / 100)) * e.qty - e.discountAmount,
        'qeyd': e.qeyd,
      }).toList(),
      'total': _total,
    };
    ...
    try {
      ...
      if (resp.statusCode >= 200 && resp.statusCode < 300) {
        ...
        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Satış sənədi təsdiqləndi')),
                                      ...
                                      Expanded(
                                        child: Column(
                                          ...
                                          children: [
                                            ...
                                            Text(
                                                  'Barkod: ${l.barcode}\n'
                                                  'Qiymət: ${l.price.toStringAsFixed(2)} AZN | Miqdar: ${_fmt18_3(l.qty)} | ${l.unit}\n'
