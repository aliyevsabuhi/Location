...
// (istəsəniz duplikat skanların qarşısını almaq üçün)
  ...
  static const String _draftKey = 'alis_draft';
                  ...
                  Expanded(
                    child: filtered!.isEmpty
                      ...
                      itemBuilder: (context, index) {
                        ...
                        final price =
                            double.tryParse(m?['alis']?.toString() ?? '') ?? 0.0;
  ...
  /// Nümunə: 2207367002200
  /// idn: bc[3..6] = 7367  (4 rəqəm)
  /// qty: bc[7..11] = 00220 → 0.220 ( /1000 )
  /*({int idn, double qty})? _parseScaleBarcode(String bc) {
    try {
      if (!_isScaleBarcode(bc)) return null;
      final idnStr = bc.substring(3, 7);
      final qtyStr = bc.substring(7, 12);
      final idn = int.parse(idnStr);
      final qty = int.parse(qtyStr) / 1000.0;
      if (qty <= 0) return null;
      return (idn: idn, qty: qty);
    } catch (_) {
      return null;
    }
  }*/
  ...
  Future<void> _submit() async {
    ...
    if (_selectedCustomerId == null || _selectedCashDeskId == null || _selectedWarehouseId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Təchizatçı/Kassa/Anbar seçilməyib.')),
      );
      return;
    }
    ...
    final payment = _paymentType == 'Nağd' ? 0 : 1; // 🔹 0=Nağd, 1=Nisyə
    ...
    final payload = {
      ...
      'lines': _lines
          .map((e) => {
        'idn': e.productId, // 🔹 server üçün məhsul id-si
        'qty': e.qty,
        'price': e.price,
        'qeyd': e.qeyd,
      })
          .toList(),
      'total': _total, // server yenə hesablayır, amma göndərmək olar
    };
    ...
    try {
      ...
      if (resp.statusCode >= 200 && resp.statusCode < 300) {
        ...
        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Alış sənədi təsdiqləndi')),
